...
[[nodiscard]] TextWithEntities UnsupportedMessageText() {
	const auto siteLink = u"https://github.com/harshi79/YoriGram/releases"_q;
	auto result = TextWithEntities{
		tr::lng_message_unsupported(tr::now, lt_link, siteLink).replace("Telegram", "YoriGram")
	};
...
HistoryMessageMarkupData UnsupportedMessageMarkup() {
	...
	row.emplace_back(
		Button::Type::Url,
		tr::lng_update_telegram(tr::now).replace("Telegram", "YoriGram"),
		Button::Visual(),
		QByteArray("https://github.com/harshi79/YoriGram/releases"));
