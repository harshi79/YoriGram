...
void BuildCategories(SectionBuilder &builder) {
	...
	builder.addSectionButton({
		.title = rpl::single(QString("YoriGram")),
...
void BuildLinks(SectionBuilder &builder) {
	...
	builder.addButton({
		.id = u"yorigram/source"_q,
		.title = rpl::single(QString("YoriGram Source")),
		.icon = { &st::menuIconChannel },
		.label = rpl::single(QString("GitHub")),
		.onClick = [=] {
			QDesktopServices::openUrl(
				QString("https://github.com/harshi79/YoriGram"));
		},
	});
	builder.addButton({
		.id = u"yorigram/upstream"_q,
		.title = rpl::single(QString("AyuGram Upstream")),
		.icon = { &st::menuIconChats },
		.label = rpl::single(QString("Credits")),
		.onClick = [=] {
			QDesktopServices::openUrl(
				QString("https://github.com/AyuGram/AyuGramDesktop"));
		},
	});
	builder.addButton({
		.id = u"ayu/crowdin"_q,
	...
	builder.addButton({
		...
		.label = rpl::single(QString("YoriGram docs")),
		.onClick = [=] {
			QDesktopServices::openUrl(
				QString("https://github.com/harshi79/YoriGram/tree/arena/01a07c24-yorigram/docs"));
		},
	});
